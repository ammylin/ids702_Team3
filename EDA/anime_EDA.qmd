...
p1 <- ggplot(top10_genre, aes(x = reorder(genres, avg_score), y = avg_score, fill = highlight)) +
  geom_col(width = 0.7) +
...
p2 <- ggplot(top10_studios, aes(x = reorder(studios, avg_score), y = avg_score, fill = highlight)) +
  geom_col(width = 0.7) +
  geom_text(aes(label = sprintf("%.2f", avg_score), color = highlight),
            hjust = -0.1, size = 3.8, show.legend = FALSE) +
...
p1 + p2 + plot_layout(ncol = 1, heights = c(5, 5))
```
