...
Likewise, we evaluated the potential for multicollinearity using the Variance Inflation Factor (VIF). The initial VIF for the interaction term was elevated (VIF = 21.45), indicating structural multicollinearity. This arises because the interaction term is calculated by multiplying the main predictors, creating a high degree of mathematical correlation between the product and the original variables. To correct for this, we mean-centered the predictors (i.e., subtracted each predictor’s mean from its value), which reduces the correlation between the interaction term and the main effects. As such, the VIF values with the centered variables dropped below 3 (Max VIF: 2.65), confirming that the initial high values were from structural multicollinearity inherent to the uncentered interaction term.
